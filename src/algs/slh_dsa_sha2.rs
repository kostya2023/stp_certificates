...
impl AlgKeypair for SLHDSA128FKeypair {
    fn generate() -> Result<Self, Error> {
        let (public, private) = sphincssha2128fsimple::keypair();
        Ok(Self {
    ...
    fn private_key_der(&self) -> Result<Vec<u8>, Error> {
        ...
        Ok(PrivateKeyInfo::new(
            ...
            AlgorithmIdentifier::new(
                ...
                Some(write_slhdsa_hasher(crate::oid::SLHDSA_HASH_SHA2.clone())),
            ),
            &self.private_key,
            None,
        )
    ...
    fn from_keypair_der(private_key: Vec<u8>, public_key: Vec<u8>) -> Result<Self, Error> {
        ...
        if hasher_oid != crate::oid::SLHDSA_HASH_SHA2.clone() {
            return Err(Error::InvalidAlgorithmError);
        }
        ...
        if hasher_oid_pub != crate::oid::SLHDSA_HASH_SHA2.clone() {
            return Err(Error::InvalidAlgorithmError);
        }
    ...
    fn sign(&self, msg: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let signature = sphincssha2128fsimple::detached_sign(msg, &secret);
    ...
    fn verify(public_key_der: &[u8], msg: &[u8], sign: &[u8]) -> Result<bool, Error> {
        ...
        sphincssha2128fsimple::verify_detached_signature(&sig, msg, &pk)
            .map(|_| true)
...
impl AlgKeypair for SLHDSA192FKeypair {
    fn generate() -> Result<Self, Error> {
        let (public, private) = sphincssha2192fsimple::keypair();
        Ok(Self {
            public_key: public.as_bytes().to_vec(),
            private_key: Zeroizing::new(private.as_bytes().to_vec()),
        })
    ...
    fn sign(&self, msg: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let sig = sphincssha2192fsimple::detached_sign(msg, &sk);
    ...
    fn verify(public_key_der: &[u8], msg: &[u8], sign: &[u8]) -> Result<bool, Error> {
        ...
        sphincssha2192fsimple::verify_detached_signature(&sig, msg, &pk)
            .map(|_| true)
            .map_err(|_| Error::VerifySignatureError("Signature invalid!".into()))
    }
...
impl AlgKeypair for SLHDSA256FKeypair {
    fn generate() -> Result<Self, Error> {
        let (public, private) = sphincssha2256fsimple::keypair();
        Ok(Self {
            public_key: public.as_bytes().to_vec(),
            private_key: Zeroizing::new(private.as_bytes().to_vec()),
        })
    ...
    fn sign(&self, msg: &[u8]) -> Result<Vec<u8>, Error> {
        ...
        let signature = sphincssha2256fsimple::detached_sign(msg, &sk);
    ...
    fn verify(public_key_der: &[u8], msg: &[u8], sign: &[u8]) -> Result<bool, Error> {
        ...
        sphincssha2256fsimple::verify_detached_signature(&sig, msg, &pk)
            .map(|_| true)
            .map_err(|_| Error::VerifySignatureError("Signature invalid!".into()))
    }
